            ...
            if(${PLATFORM}_VARIANTS_PATH)
                ...
                foreach(dir ${sub-dir})
                    if(IS_DIRECTORY ${dir})
                        ...
                        set(variant.${variant}.path ${dir} CACHE INTERNAL "The path to the variant ${variant}")
                    endif()
                endforeach()
            endif()
            ...
            if(${PLATFORM}_CORES_PATH)
                ...
                foreach(dir ${sub-dir})
                    if(IS_DIRECTORY ${dir})
                        ...
                        set(core.${core}.path ${dir} CACHE INTERNAL "The path to the core ${core}")
                    endif()
                endforeach()
            endif()
...
function(get_energia_flags COMPILE_FLAGS_VAR LINK_FLAGS_VAR BOARD_ID MANUAL)
    ...
    if(BOARD_CORE)
        ...
        if(NOT MANUAL)
			set(COMPILE_FLAGS "${COMPILE_FLAGS} -I\"${core.${BOARD_CORE}.path}\"")
        ...
        if(ENERGIA_VER VERSION_GREATER 15 OR ENERGIA_VER VERSION_EQUAL 15)
            if(NOT MANUAL)
                set(PIN_HEADER ${variant.${${BOARD_ID}.build.variant}.path})
                if(PIN_HEADER)
...
function(setup_energia_core VAR_NAME BOARD_ID)
    ...
    if(BOARD_CORE)
        if(NOT TARGET ${CORE_LIB_NAME})
            set(BOARD_CORE_PATH ${core.${BOARD_CORE}.path})
